class PickAMovie < ActiveRecord::Base
  ...
  def self.tickets
    #get the current movie and view from Application.rb
    ...
    if purchase_prompt == "Purchase a ticket for #{current_movie.title}"
      ...
      showtime_prompt = showtime_selection.select(
        #match showtime to appropriate movie showtimes and location
